...
def run_linear(
    ...
    if weights is not None:
        ...
        linear.load_state_dict(state_dict=state,strict=True)
...
def run_embedding(
    ...
    embed=Embedding(num_embeddings=vocab_size,embedding_dim=d_model,device=token_ids.device)
    if weights is not None:
        state={'weight':weights}
        embed.load_state_dict(state,strict=True)
    embeds=embed(token_ids)
    return embeds
...
def run_swiglu(
    ...
    pw_ffn=SwiGLU(d_model=d_model,d_ff=d_ff,device=in_features.device,dtype=in_features.dtype)
    state={"W1":w1_weight,'W2':w2_weight,'W3':w3_weight}
    pw_ffn.load_state_dict(state)
    out=pw_ffn(in_features)
    return out
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    from cs336_basics.model.modules import Multihead_self_attention,RotaryPositionEmbedding
    MultiheadSA=Multihead_self_attention(d_model=d_model,num_heads=num_heads,theat=theta,max_seq_len=max_seq_len,position_embedding=RotaryPositionEmbedding,device=in_features.device,dtype=in_features.dtype)
    state={'q.weight':q_proj_weight,'k.weight':k_proj_weight,'v.weight':v_proj_weight,'o.weight':o_proj_weight}
    MultiheadSA.load_state_dict(state)
    out=MultiheadSA(in_features,token_positions)
    return out
...
def run_rope(
    ...
    RoPE=RotaryPositionEmbedding(theat=theta,d_k=d_k,max_seq_len=max_seq_len)
...
def run_transformer_block(
    ...
    block=tb(d_model=d_model,num_heads=num_heads,d_ff=d_ff,theat=theta,max_seq_len=max_seq_len,device=in_features.device,dtype=in_features.dtype)

    state = {
    'mha.q.weight': weights['attn.q_proj.weight'],
    'mha.k.weight': weights['attn.k_proj.weight'],
    'mha.v.weight': weights['attn.v_proj.weight'],
    'mha.o.weight': weights['attn.output_proj.weight'],
...
def run_transformer_lm(
    ...
    state = {
        ...
        'out_linear.weight': weights['lm_head.weight'],
    }

    for i in range(num_layers):
        state[f'blocks.{i}.mha.q.weight']  = weights[f'layers.{i}.attn.q_proj.weight']
        state[f'blocks.{i}.mha.k.weight']  = weights[f'layers.{i}.attn.k_proj.weight']
        state[f'blocks.{i}.mha.v.weight']  = weights[f'layers.{i}.attn.v_proj.weight']
        state[f'blocks.{i}.mha.o.weight']  = weights[f'layers.{i}.attn.output_proj.weight']
        state[f'blocks.{i}.norm1.weight']  = weights[f'layers.{i}.ln1.weight']
        state[f'blocks.{i}.norm2.weight']  = weights[f'layers.{i}.ln2.weight']
...
def run_rmsnorm(
    ...
    rmsnorm=RMSNorm(d_model=d_model,eps=eps,device=in_features.device)
    if weights is not None:
        state={'weight':weights}
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    gradient_clipping(params=parameters,max_l2=max_l2_norm)
...
def run_save_checkpoint(
    ...
    """
    from cs336_basics.check_point import save_checkpoint
...
def run_load_checkpoint(
    ...
    """
    from cs336_basics.check_point import load_checkpoint
